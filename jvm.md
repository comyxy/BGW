...
### 引用、软引用Soft、弱引用Weak、虚引用Phantom的区别？

* 引用：```Object a = new Object()```
* 软引用：内存不足时GC回收
* 弱引用：GC回收
* 虚引用：必须和一个引用队列相关联，当引用的对象被回收之前，虚引用会进入这个队列。DirectByteBuffer堆外内存```Cleaner```回收机制。

### 如何回收堆外内存？

堆外内存的引用被 JVM 堆上的对象持有，线程栈通过持有堆内的对象，间接操作堆外内存。对堆外内存的回收，也就是在 gc 回收堆内对象时，手动释放对象持有的堆外内存。
通过虚引用监控堆内对象的回收，并同时回收堆外内存。Java 提供了 Cleaner 类，通过虚引用绑定对象，并注册回收时的回调函数，来简化这种操作。
...
1. 新生代用来存放新对象，活过 gc 的对象少，因此使用*复制copy*算法来筛选出少量存活的对象。
2. 老年代用来存放寿命长的对象，死于 gc 的对象少，因此使用*标记整理/清除sweep*算法来筛选出少量死亡的对象。
...
4. *CMS*
   ...
   * 并发收集器，可与用户线程同时工作
   * 标记清除
   * 流程：（三色标记：白色未标记，灰色自己已标记，黑色自己以及引用节点都已标记）
...
5. *G1*（Garbage First 估计每个Region中的垃圾⽐比例例，优先回收垃圾多的Region）
   * 不依赖分代，而是将整个堆分为不同区块，回收时按照 STW 期望时间选择回收优先级最高（回收价值最大）的区块：Eden、Survivor、Old、Humongous
   * 年轻代：STW、Copy、Parallel，老年代：Mostly-concurrent marking、Incremental compaction
   * 适合于多处理器大容量内存的服务器，能设置参数来调节 STW 和 吞吐量的平衡，能充分使用多核 CPU 来降低 STW，有毫秒级的 STW 时间模型
   * 处理不同Region之间的引用
     * CardTable与RemeberedSet(RS)
   * Fully Young GC（E区域不够分配一个对象）与Mixed GC（堆内存使用达到一个阈值）
     * Fully Young GC（STW）
       * 构建CollectionSet（E+S）
       * 扫描GC Roots
       * Update RS：排空Dirty Card Queue更新RS引用信息
       * Process RS：找到哪些对象被其他Region对象引用
       * Object copy
...
2. 分配内存
   * 保证线程安全
     ...
     * CAS自旋：超过 TLAB 的部分，使用*CAS自旋*的方式分配堆上内存
3. 初始化零值
4. 设置对象头
5. 执行init方法
